    ...
    <div className="space-y-6 animate-in fade-in duration-700">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500">Manage monthly disbursements and salary slips.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <GlassCard className="p-4 flex items-center gap-4">
          ...
          <div><p className="text-sm text-slate-500">Total Staff</p><p className="text-2xl font-bold text-slate-800">42</p></div>
        </GlassCard>
        <GlassCard className="p-4 flex items-center gap-4">
          <div className="p-3 bg-green-500/20 text-green-400 rounded-lg"><DollarSign /></div>
          <div><p className="text-sm text-slate-500">Total Disbursed</p><p className="text-2xl font-bold text-slate-800">$124,500</p></div>
        </GlassCard>
        <GlassCard className="p-4 flex items-center gap-4">
          <div className="p-3 bg-amber-500/20 text-amber-400 rounded-lg"><AlertCircle /></div>
          <div><p className="text-sm text-slate-500">Pending Clearances</p><p className="text-2xl font-bold text-slate-800">3</p></div>
        </GlassCard>
      </div>

      <GlassCard className="p-0 overflow-hidden">
        <div className="p-4 border-b border-slate-200 relative">
          <Search className="absolute left-7 top-1/2 -translate-y-1/2 text-slate-500 w-4 h-4" />
          <input type="text" placeholder="Search staff..." className="w-full max-w-md bg-black/5 border border-slate-200 rounded-md py-2 pl-9 pr-4 text-sm text-slate-800 focus:outline-none" value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        <table className="w-full text-left">
          <thead className="bg-black/5 border-b border-slate-200">
            <tr>
              <th className="p-4 text-slate-500 font-medium">Emp ID</th>
              <th className="p-4 text-slate-500 font-medium">Name</th>
              <th className="p-4 text-slate-500 font-medium">Role</th>
              <th className="p-4 text-slate-500 font-medium text-right">Base Salary</th>
          ...
          <tbody>
            {staffList.map(staff => (
              <tr key={staff.id} className="border-b border-slate-200 hover:bg-black/5">
                <td className="p-4 text-slate-500">{staff.id}</td>
                <td className="p-4 text-slate-800 font-medium">{staff.name}</td>
                <td className="p-4 text-slate-500">{staff.role}</td>
                <td className="p-4 text-slate-800 text-right">${staff.salary.toLocaleString()}</td>
