    ...
    <div className="min-h-screen bg-[#0A0A0C] text-slate-100 p-6 md:p-10 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-white/[0.08] pb-6">
        ...
        <div className="mt-4 md:mt-0 flex items-center gap-3 bg-white/[0.03] border border-white/[0.08] rounded-xl px-4 py-2">
          ...
          <span className="text-sm font-medium text-slate-300">RLS Boundary Enforced</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="bg-[#111115]/80 border border-white/[0.06] rounded-2xl p-6 relative overflow-hidden backdrop-blur-xl">
            <DollarSign className="absolute right-4 top-4 w-12 h-12 text-emerald-500/10" />
            <h3 className="text-sm font-medium text-slate-400">Total Incoming Cash</h3>
          ...
          <div className="bg-[#111115]/80 border border-white/[0.06] rounded-2xl p-6 relative overflow-hidden backdrop-blur-xl">
            ...
            <h3 className="text-sm font-medium text-slate-400">Total Operating Expenses</h3>
          ...
          <div className="bg-[#111115]/80 border border-white/[0.06] rounded-2xl p-6 relative overflow-hidden backdrop-blur-xl">
            ...
            <h3 className="text-sm font-medium text-slate-400">Net Campus Surplus</h3>
        ...
        <div className="bg-[#111115]/80 border border-white/[0.06] rounded-2xl p-6 backdrop-blur-xl flex flex-col items-center justify-center">
          <h3 className="text-sm font-semibold tracking-wider text-slate-400 uppercase self-start">Revenue Allocation</h3>
          ...
          <div className="grid grid-cols-2 gap-4 w-full mt-4">
            {pieData.map((p) => (
              <div key={p.name} className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-300 font-medium">{p.name}: ${p.value.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-[#111115]/80 border border-white/[0.06] rounded-2xl p-6 backdrop-blur-xl">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold tracking-wider text-slate-400 uppercase">Ustad Teaching Standings</h3>
              ...
              <div key={u.id} className="flex items-center justify-between bg-white/[0.02] border border-white/[0.04] rounded-xl p-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">{u.pages_taught} pages taught</p>
                  </div>
                </div>
      ...
      <div className="bg-[#111115]/80 border border-white/[0.06] rounded-2xl p-6 backdrop-blur-xl">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-sm font-semibold tracking-wider text-slate-400 uppercase">Student Metrics Matrix</h3>
            ...
            <tbody className="divide-y divide-white/[0.04] text-sm">
              {data.student_ratings.map((s) => (
                <tr key={s.id} className="hover:bg-white/[0.01] transition-colors">
                  ...
                  <td className="py-4 text-right text-xs text-slate-400 font-medium">Active Roster</td>
                </tr>
              ))}
            </tbody>
          </table>
